...
my %TERMSIZE =
    (
     $X11 => { $SMALL => 800, $MEDIUM => 1200,  $LARGE => 4800  }, # screen pix
     $PS  => { $SMALL => 1,   $MEDIUM => 2,    $LARGE => 3    }, # pages
     $PNG => { $SMALL => 800, $MEDIUM => 1200, $LARGE => 4800 }  # image pix
     );
...
my $OPT_size      = $MEDIUM;       # -small, -medium, -large
my $OPT_SNP;                       # -S option
